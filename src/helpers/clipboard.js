...
class ClipboardManager {
  ...
  _readPortalToken() {
    try {
      ...
      try {
        const token = fs.readFileSync(this._getLegacyPortalTokenPath(), "utf8").trim() || null;
        if (token) this._savePortalToken(token);
        return token;
      } catch {
        return null;
      }
  ...
  showAccessibilityDialog(testError) {
    ...
    if (isStuckPermission) {
      dialogMessage = `🔒 VoiceLab needs Accessibility permissions, but it looks like you may have OLD PERMISSIONS from a previous version.

❗ COMMON ISSUE: If you've rebuilt/reinstalled VoiceLab, the old permissions may be "stuck" and preventing new ones.

🔧 To fix this:
1. Open System Settings → Privacy & Security → Accessibility
2. Look for ANY old VoiceLab entries and REMOVE them (click the - button)
3. Also remove any entries that say "Electron" or have unclear names
4. Click the + button and manually add the NEW VoiceLab app
...
Would you like to open System Settings now?`;
      ...
      dialogMessage = `🔒 VoiceLab needs Accessibility permissions to paste text into other applications.
...
4. Add VoiceLab to the list and check the box
5. Restart VoiceLab
